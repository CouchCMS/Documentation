...
import doNotImg from './img/do-not.png';
import doNot2Img from './img/do-not2.png';
...
This part of our application has been built using the [**Extended Users**](../../../tags-reference/extended-entities/extended-users/) module.
...
The process of setting up the user accounts template is described in detail in the docs [**Extended Users**](../../../tags-reference/extended-entities/extended-users/) but I'll repeat the steps here for the sake of completeness:
...
2. Coming back to the admin-panel, rename its default page to **"DO NOT DELETE"** and unpublish it.
    ...
    <Image src={doNotImg} alt="Default page renaming" />
    > Renaming the default page to "DO NOT DELETE"

    <Image src={doNot2Img} alt="Unpublishing the page" />
...
5. Test that the extended users functionality works correctly by following these steps:

    a. Go to the admin panel and create a cloned page from the `users/index.php` template. Verify that a corresponding user account is automatically created.

    b. Next, open the super-admin account for editing. Simply click 'save' without changing anything. This should cause a corresponding cloned page to be created in the users template.
...
Fortunately, registering these templates is a much simpler affair. Visit the following pages as super-admin:
...
:::danger[Expected Errors]
When you first visit these template URLs, you may see errors similar to these:

```txt
Error embedding file: /var/www/html/snippets/views/pads/.html
Error embedding file: /var/www/html/snippets/views/notes/.html
```

These errors are expected and will disappear once you've completed the template registration process.
:::

After visiting both URLs, return to the admin panel. You should now see both templates appearing in the sidebar, alongside the user template we registered earlier.

To verify that the registration was successful, navigate back to `https://www.yoursite.com/notes.php`. The previous errors should be gone, and you should see the application interface.
