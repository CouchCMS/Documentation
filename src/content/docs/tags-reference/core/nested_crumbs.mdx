...
The **nested_crumbs** tag can be used to easily create breadcrumbs out of [**nested-pages**](../../../concepts/nested-pages/).

:::note
There is another tag named [**breadcrumbs**](../breadcrumbs/), which is used with the folder hierarchy associated with normal cloned pages. The `nested_crumbs` tag on the other hand works with nested-pages only.
:::
...
The 'Advanced Settings' of each nested-page has a checkbox labeled 'Show in menu'. If that is unchecked, the page is not included by default in the breadcrumbs. You can override this by setting `ignore_show_in_menu` parameter to `1`.
