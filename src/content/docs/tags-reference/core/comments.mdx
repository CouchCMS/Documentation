...
title: "comments"
category: "tag"
description: "Reference documentation for the comments tag used to fetch, filter, and display user comments from CouchCMS pages with pagination support."
keywords:
    - comments
    - CouchCMS
    - Couch
...
Please see [**Core Concepts - Comments**](../../../concepts/comments/) for a full discussion of the Comments tag.
...
Fetch ALL approved comments but show only 5 at one time. To move to the next 5 or the previous 5 comments, pagination code has to be used (see [**paginator**](../paginator/)).
...
In addition to these variables, the pagination related variables that describe the current status of the loop are also set. Please see [**paginator**](../paginator/) for details.
