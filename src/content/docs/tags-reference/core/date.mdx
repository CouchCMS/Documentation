...
The date to be formated. This parameter is expected to be in `Y-m-d H:i:s` format (e.g. 2010-05-30 21:35:54). All date related variables set by Couch tags, e.g. `k_page_date` etc., are in this format.


### `format`

The `date` tag supports two different types of format characters - locale-aware and non locale-aware. With locale-aware characters, you can specify that the date is to formatted according to, for example, `french` locale or `italian` locale by setting the `locale` parameter. The locale-aware characters all have a % sign prefixed to them.
...
Some locales do not provide their output in UTF8 character set. This causes strange ?? characters to appear in the output. The `date` tag can help converting the output to UTF8 if you can provide it with information about the charset used by the locale.
