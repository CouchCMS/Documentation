---
title: Search
description: "Complete guide to implementing search functionality in CouchCMS using MySQL fulltext search with relevance ranking and pagination support."
keywords:
    - search
    - fulltext search
    - search forms
...
The tag that Couch uses to find pages containing the searched word/words is the [**search**](../../tags-reference/core/search/) tag. This tag is very similar to the [**pages**](../../tags-reference/core/pages/) tag discussed before in the way it fetches the relevant pages and then steps through each of them setting up variables that describe the current page.

Two of the parameters supported by this tag are - `masterpage` and `limit`. By default, the [**search**](../../tags-reference/core/search/) tag searches through pages of all the available templates. If you wish to set up search for only a section of your website, use the `masterpage` parameter to make Couch search only certain templates or exclude certain templates.
...
In the example above, we are using the [**gpc**](../../tags-reference/core/gpc/) tag to get a querystring parameter named `s` from the URL and set its value as the keywords to be searched, thus making the example behave exactly as that with the `search` tag not having any keywords explicitly specified.
...
Couch has a simple tag named [**search_form**](../../tags-reference/core/search_form/) that generates such a form for you that has a textfield named `s`:

```php title="Basic Search Form"
<cms:search_form />
```

The snippet above will generate a search form that when submitted will invoke the same page the snippet was called from, after putting the searched terms in the querystring. Of course you are expected to place a [**search**](../../tags-reference/core/search/) tag on the same page to handle the search.

:::tip[Custom Forms]
You can create and use your own form instead of using the one created by [**search_form**](../../tags-reference/core/search_form/) tag. Just make sure that the textbox, within which the keywords will be inputted by the users, is named `s`.
:::

Sometimes one wishes to initiate the search from one page and display the results in another page. For example you might wish to place the search form on the homepage (`index.php`) of the website but want that on submitting this form the user is taken to another page (`search.php`) that then displays the result of the search. You can do this by placing the [**search_form**](../../tags-reference/core/search_form/) tag within `index.php` and placing the [**search**](../../tags-reference/core/search/) tag snippet within `search.php`. To make the [**search_form**](../../tags-reference/core/search_form/) tag invoke `search.php` (instead of `index.php`) upon form submission, its `processor` parameter can be set in the following manner:
