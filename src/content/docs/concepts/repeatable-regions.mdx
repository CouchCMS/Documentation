...
description: "Complete guide to creating and managing dynamic repeatable regions in CouchCMS templates for portfolios, image galleries, and sortable content arrays."
keywords:
    - repeatable regions
    - dynamic content
    - portfolio
...
import repeatableRegions1 from '../../../assets/img/contents/repeatable-regions-1.png';
import repeatableRegions2 from '../../../assets/img/contents/repeatable-regions-2.png';
import repeatableRegions3 from '../../../assets/img/contents/repeatable-regions-3.png';
import repeatableRegions4 from '../../../assets/img/contents/repeatable-regions-4.png';
...
2. Enclose the editable region defined above with a newly introduced tag - [**repeatable**](../../tags-reference/core/repeatable/):
...
<Image src={repeatableRegions1} alt="Single Image Field" />
...
<Image src={repeatableRegions2} alt="Two Image Fields" />
> Two image fields after clicking the "Add item" button

You are free to add as many images as you desire:

<Image src={repeatableRegions3} alt="Multiple Image Fields" />
...
Well, no! Of the 12 native types of editable regions in Couch, the [**repeatable**](../../tags-reference/core/repeatable/) tag supports the following 8:
...
Instead of using the `richtext` type we can now use a new type of editable region named [**nicedit**](../../tags-reference/core/editable/nicedit/). The `nicedit` type uses a very light-weight wysiwyg editor of the same name. It is not as full-featured as the CKEditor that `richtext` uses however it gets the job done well and is light enough to be repeated any number of times on a single page (we'll find an example of its use below).

**We can also use 'nicedit' type as a regular editable region (i.e. as the usual non-repeatable regions)**.

As for the `thumbnail` type, we do not have any substitute editable region for it but v1.3 has a new tag named [**thumbnail**](../../tags-reference/core/thumbnail/) that does an identical job. Please see [**thumbnail**](../../tags-reference/core/thumbnail/) documentation for this tag's use.
:::
...
<Image src={repeatableRegions4} alt="Image with Description Fields" />
...
Since the repeatable editable regions now actually form a 'composite' entry (i.e. do not have a single value), we have to use a new tag named [**show_repeatable**](../../tags-reference/core/show_repeatable/) as a helper to make the individual values available to us.
...
Please note that the [**show_repeatable**](../../tags-reference/core/show_repeatable/) tag has to be given the name associated with a [**repeatable**](../../tags-reference/core/repeatable/) tag (take care of using the quotes).
It iterates through each row of regions in the set making available the values within each constituent region. These values then can be displayed using the regular [**show**](../../tags-reference/core/show/) tag.
