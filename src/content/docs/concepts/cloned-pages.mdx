...
To make these pages updatable by the users, it is sufficient to mark out the updatable HTML areas in the templates using the [**editable**](../../tags-reference/core/editable/) tags and Couch will make them editable.
...
A template can be marked as being clonable by using the [**template**](../../tags-reference/core/template/) tag somewhere within it and setting its `clonable` parameter to `1`. Place the following snippet somewhere at the beginning of the template (within the `<head>` tag will do just fine):
...
The above page is unequivocally one single page.
