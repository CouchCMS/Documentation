...
**One editable region is mandatory**, though. It is the one that is used to input the product's price and **has to be named '*pp\_price*'**.
...
This is all routine stuff. The display of the product options (variants) will, however, require the use of a new tag - __*pp\_product\_options*__.
...
CouchCart simplifies this task for you by providing a handy tag named __*pp\_product\_form*__ that takes care of all the nitty-gritty.
...
Note that the 'action', 'method' and 'accept-charset' parameters are not needed now because __*pp\_product\_form*__ will handle those.
...
To make it easy for us to display this information, CouchCart has a tag named __*pp\_selected\_options*__.
...
**2\.** Set the 'name' of the input element to 'qty'. Since in this FORM, there can be multiple items and hence multiple quantity input boxes, we make the name an array. So the name we give to the input element is **qty\[&lt;cms:show line\_id /&gt;\]** (where '*line\_id*' is a unique ID allocated to each item in the cart by CouchCart).
...
CouchCart provides a tag named __*pp\_count\_items*__ that gives us this number.
...
**1\.** As an HTML 'input' or 'button' of type 'submit' within the form generated by pp\_cart\_form.
It is mandatory to name this 'input' or 'button' element as 'checkout' for this method to work.

**2\.** As a simple anchor '&lt;A&gt;' element. The value for its href attribute can be generated using the pp\_checkout\_link tag
...
The only additional bit of code is the __*no\_cache*__ tag to make sure Couch never caches the output of this template.
...
To explain the code above, the __*pp\_payment\_gateway*__ tag, by default, empties the cart before redirecting to PayPal. As we are still in the phase of testing the cart, it might be a good idea to prevent it from doing so. The 'empty\_cart' parameter set to '0' switches off this behaviour.
...
In fact, you can even give the buyer a choice of either using PayPal or pay using a bank check etc. If the choice is a bank check, we can skip invoking pp\_payment\_gateway and show him the seller's bank details etc. as the final step of the transaction.
...
You'll find a JavaScript file named '_cart-modal.js_' in the 'js' folder of our site.
