...
function generatePRTitle(commits, categorizedFiles) {
    ...
    const hasFix = commitMessages.some((m) => m.startsWith("fix"));
    const hasDocs = commitMessages.some((m) => m.startsWith("docs"));
    ...
    } else if (updatedPages.length > 3) {
        if (hasFix) {
            return "fix: correct multiple documentation issues";
        }
        return `docs: update ${updatedPages.length} documentation pages`;
    } else if (hasFix) {
