...
- Tag References:
    ...
    - Use bold and lowercase: [**editable**](../../tags-reference/core/editable/)
    - Never capitalize tag names in references
    - Include the link to reference documentation
    - Example: [**show**](../../tags-reference/core/show/), [**template**](../../tags-reference/core/template/)
...
- Documentation References:
    ...
    - Only tags-reference uses lowercase: [**editable**](../../tags-reference/core/editable/)
...
- CRITICAL: Never modify code block titles that contain filenames:

    - If title shows a filename like `php title="blog.php"`, keep it exactly as is
    - Only add descriptive titles for code blocks without filenames
    - Preserve the exact filename, extension and quotes in the title
...
- CRITICAL: Never modify diff code blocks:

    - Always preserve the exact content of diff blocks
    - Never remove or change the '+' and '-' markers
    - Keep the exact indentation and spacing
    - Preserve all line breaks
    - Example of a diff block to preserve exactly:
...
- IMPORTANT: Never remove lines with '+' or '-' at the beginning of lines in diff code blocks
...
### 2.5. Components and UI Elements
...
The Card component has specific formatting rules:

- Never indent the Card component or its content
- Keep original spacing and line breaks
- Example:

```markdown title="Card Example"
...
### 2.6. Media and Assets

**General requirements:**

- Alt text is required, descriptive, and functional
- Max width: 1200px for screenshots, 300px for thumbnails
- WebP preferred, PNG/JPG as fallback
- Optimize for web
- Do not use images for text content
- Ensure sufficient resolution
- Use meaningful filenames
- Place images in the correct subdirectory:
    - `/assets/img/contents/` for content images
    - `/assets/img/ui/` for UI elements
    - `/assets/img/diagrams/` for diagrams

**File format:**

- Filename: `feature-action-context.webp`
- Alt text: short description of the content
- Caption: optional, explanation or context, placed directly below the image with `>`
...
- Always use the Astro `<Image />` component:
    ...
    - Use in the content:
        ```mdx
        <Image src={img1} alt="Description" />> Caption directly below the image
        ```
- Alt text is required.
- Caption always directly below the image with a `>`.

**Usage in `.md` files:**

- Use classic markdown image syntax:
...
<Image src={loginImg} alt="Login screen" />> Login screen of the application
```
...
- Never use markdown image syntax in `.mdx` files.
- Caption must always be directly below the image, using a `>` blockquote.
- See the directory example below for structure.
...
- Align columns consistently:
    ...
    | Left     |  Center  |    Right |
    ```
- Use hyphen (-) for empty cells
- Minimum 3 dashes for header separation
...
### 2.9. Admonitions and Callouts

Use appropriate admonition types:

```markdown
:::note[Context]
For background information or prerequisites
:::
...
:::caution[Important]
For version-specific or limitation notes
:::
...
- Use consistent formatting
- Choose appropriate type based on content importance
...
### 3.1. Line Length and Wrapping

- Maximum line length: 80 characters for content
- Code blocks may extend to 120 characters
- Use soft wrapping for long content
- Break URLs between path segments

### 3.2. Indentation and Spacing

- Always use 4 spaces for indentation everywhere EXCEPT for code blocks
...
- Use 4 spaces for code blocks
- Use 4 spaces for blockquotes
- Use 4 spaces for admonitions
